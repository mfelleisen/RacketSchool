...
@bold{Meta-functions} Language models need meta-functions in addition to
reduction relations. We have seen one such function in action:
...
Language modelers want to define their own meta-functions. One example 
is the evaluator: 
@;%
@(begin
#reader scribble/comment-reader
...
we can escape to Racket via a comma (unquote). 

Look what happens if we use @racket[eval] on @tt{(y (lambda (x) x))}. It
...
clauses with extra conditions and (2) a function that retrieves the free
...
@;%
Note how @racket[eval-2] has two clauses and how the first clause mentions
the function @racket[fv] (short for ``free
variables''). @margin-note*{Compilers often call this ``undefined
variable.''}
...
@bold{TDD} But we really want test-driven development of these
